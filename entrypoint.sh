...
rsync -e "ssh -v -p 22 -i $SSH_PRIVATE_KEY_PATH -o UserKnownHostsFile=$SSH_KNOWN_HOSTS_PATH" --delete -a --exclude={'.*','.*/','robots.txt','.github','.git','.gitignore'} $SOURCE_PATH $TARGET_PATH
